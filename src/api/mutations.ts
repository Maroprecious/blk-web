...
export function useOrders() {
  return useMutation({
    mutationFn: (data: CreateOrderDto) =>
      api.post("/orders", data).then((resp) => resp.data),
  });
}
...
export function usePaymentInit() {
  return useMutation({
    mutationFn: (data: addPayload) =>
      api.post("/payments/init", data).then((resp) => resp.data),
  });
}
...
export function useLogout() {
  return useMutation({
    mutationFn: (data: logOutPayload) =>
      api.post("/auth/signout", data).then((resp) => resp.data),
  });
}
export function useAddProduct() {
  return useMutation({
    mutationFn: (data: addProductPayload) =>
      api.post("/products", data).then((resp) => resp.data),
  });
}
export function useCreateRetreat() {
  return useMutation({
    mutationFn: (data: addProductPayload) =>
      api.post("/retreats", data).then((resp) => resp.data),
  });
}
